	...
	internal static class Svg
	{
		internal static void Save(XmlWriter writer, IEnumerable<IShape> shapes, Size size)
		{
			...
			if (shapes != null)
				...
				{
					var persister = ShapeDictionary.GetHelper(s.GetType().Name) as IPersistableHelper;
					if (persister != null)
		...
		private static string SafeString(double f)
		{
			return f.ToString(CultureInfo.InvariantCulture).Replace(',', '.');
		}

		internal static void WriteRoundedRectangle(XmlWriter writer, Point location, Size size, Color fill, Pen pen,
			int radius)
		{
			writer.WriteStartElement("rect");
